...
- `src/data/renderedContent.test.tsx` records every word the page and the printed CV render, with the three numbers read off the clock replaced first. They were in the recorded text, which made the snapshot a function of the day it ran: `12+ years of experience` becomes thirteen on 1 January 2027 and the copyright year turns over every New Year, so the suite was going to fail on a date with no change to the code — and the obvious response, `vitest -u`, silently re-records all 33 kB of page text, which is the one thing the snapshot exists to prevent. Replaced rather than frozen, because a frozen clock leaves the dependency in place and invisible. Two renders five years apart prove the values really move and that removing them leaves the same text; what the numbers are is asserted against the functions that compute them.
...
- `tests/printQrCard.test.ts` draws the card the build serves at `/cv-qr-code.png`, decodes the PNG, samples the centre of every one of its 841 modules and rebuilds the matrix, failing if it is not the code for `PRINT_URL`. That address is on material this site does not control and cannot recall, and it answered 404 from the day the two files behind it — a PNG and an SVG, for a QR the print template had stopped rendering — were removed as assets nothing pointed at. It comes back drawn at build time rather than committed, because a generated artifact checked into a repository is free to drift from what generated it, and it is written straight into the build output and never into `public/`, which is why it needs no exemption from the ratchet that removed its predecessors. That leaves it the one shipped artifact here with no other guard: no import reaches it and the unused-asset check cannot see it, so this reads the image back instead of checking that a file was written. Three things it holds that nothing else could: that the code still encodes the address, that the white margin a scanner needs is still around it — the CV's own drawing has none, and on paper the sheet supplies one by accident — that the text has not run off the card, which at a fixed 58px it did, silently, because drawing text does not refuse to overflow; and that the code still sits in the site's gradient frame, running cyan into purple, which is the one thing on the card that says whose the code is and which nothing asserted when it was added — taking the frame to nothing left every other check green. The frame is sampled inside its own band, because the glow spreads the same colours over the panel for another forty pixels and a probe out there would pass on the glow alone. It also reads the plugin list out of `vite.config.ts`, since everything else here judges a card the test drew itself and would stay green with the plugin unhooked and the address answering 404 again. Two of its checks were tautologies when written and were caught by mutation: the size was compared against the constant that produced it, and the code was drawn from an address the test supplied rather than the one the build ships.
- `tests/sitemap.test.ts` holds `public/sitemap.xml` to the plugin in `vite.config.ts` that stamps it at build: every entry dated from the build, the rest of the document untouched, and the same result on a second pass. The assertion that matters least on a green day is the last one — without an entry to rewrite the plugin does nothing at all, and the dates go back to being whatever someone last typed.
