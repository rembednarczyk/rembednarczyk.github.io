...
- `scripts/runPrintCheck.ts` prints the site to PDF and reads the sheets back, failing if the CV runs past five of them or if any sheet but the last is more than a fifth blank at the foot. It ran to six sheets with 38% of the first and 58% of the fourth empty, because every section and job entry carried `break-inside-avoid`: a block taller than the space left on a page does not shrink to fit, it moves whole and leaves the gap behind. Nothing on a screen shows any of this.
