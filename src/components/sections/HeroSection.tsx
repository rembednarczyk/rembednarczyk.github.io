  ...
  return (
    <section className="relative min-h-[80vh] flex flex-col justify-center items-start">
      ...
      >
        ...
        <h1 itemProp="name" className="text-5xl md:text-7xl font-extrabold tracking-tight mb-4 text-white print:hidden">
          {heroData.name}
        </h1>
        <h2 itemProp="jobTitle" className="text-4xl md:text-6xl font-bold tracking-tight mb-6 text-slate-400 print:text-2xl print:text-black print:mb-4">
          {heroData.subtitle}
        </h2>
        <p itemProp="description" className="max-w-2xl text-lg text-slate-400 mb-10 leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 max-w-3xl">
          {heroData.metrics.map((metric, i) => (
            <div key={i} className="group relative bg-[#0a1128]/50 backdrop-blur-sm border border-white/10 rounded-2xl p-5 flex flex-col items-center justify-center text-center hover:bg-white/5 hover:border-cyan-400/50 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(34,211,238,0.15)] active:scale-95 active:bg-white/5 active:border-cyan-400/50 active:shadow-[0_0_20px_rgba(34,211,238,0.15)] transition-all duration-300 overflow-hidden cursor-pointer">
              ...
              <span className="relative text-3xl md:text-4xl font-black text-white group-hover:text-cyan-400 group-active:text-cyan-400 transition-colors mb-2 tracking-tight">{metric.value}</span>
              <span className="relative text-[10px] sm:text-xs text-slate-400 uppercase tracking-widest font-semibold leading-tight">{metric.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          {heroData.tags.map((tech, i) => (
            <span key={i} className="px-3 py-1 text-sm font-mono text-cyan-400 bg-cyan-400/10 border border-cyan-400/20 rounded-full">
              {tech}
            </span>
          ))}
        </div>
